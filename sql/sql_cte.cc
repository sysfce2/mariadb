...
/**
  @brief
  Allocate on statement memory a list of identifier names in the supplied
  item list
*/
...
List<Lex_ident_sys> *allocate_and_save_item_names(THD *thd,
                                                  List<Item> &item_list)
{
  List<Lex_ident_sys> *result;

  if (unlikely(!(result= new(thd->stmt_arena->mem_root) List<Lex_ident_sys>)))
    return nullptr;
