...
public:
  ...
  /** Set the block as modified after changing all the pages. */
  void set_modify_blocks(mtr_t *mtr)
  {
    for (uint32_t i= 0; i < m_old_xdes_pages.size(); i++)
    {
      if (m_old_xdes_pages[i] == nullptr) continue;
...
/** Write a file address in the given block address.
In case of temporary tablespace, write the data into
the page directly.
@param 	space_id	tablespace to be shrinked
@param	block		file page
@param	faddr		file address location
@param	page		page number
@param	boffset		byte offset
@param	mtr		mini-transaction */
static
void fsp_lst_write_addr(
  const buf_block_t &block, byte *faddr,
  uint32_t page, uint16_t boffset, mtr_t *mtr)
{
  if (mtr->is_logged())
  {
    flst_write_addr(block, faddr, page, boffset, mtr);
    return;
  }
  ...
  uint32_t skip_len, mtr_t *mtr)
{
  ...
  if (last_valid_addr.page == FIL_NULL)
  {
    ...
    the PREV pointer of last valid descriptor with FIL_NULL */
    fsp_lst_write_addr(
      *header,
  ...
  else mach_write_to_4(len_bytes, len - skip_len);
  return DB_SUCCESS;
}
...
func_exit:
    if (hdr_offset == FSP_FREE_FRAG + FSP_HEADER_OFFSET)
    {
	  ...
	  mtr->write<4>(*header, frag_used_byte,
          ...
          mach_write_to_4(
            frag_used_byte, n_used_frag - ((orig_len - len) * 2));
      }
    }
    return DB_SUCCESS;
  }
  ...
  if (cur_addr.page == FIL_NULL)
  {
    ...
    {
      memset(
        header->page.frame + FIL_ADDR_PAGE + hdr_offset + FLST_FIRST,
        0xff, 4);
      memset(
        header->page.frame + FIL_ADDR_PAGE + hdr_offset + FLST_LAST,
        0xff, 4);
    }
  }
  else
  ...
  else mach_write_to_4(len_bytes, len);
  goto func_exit;
}
                        ...
                        uint32_t threshold, mtr_t *mtr)
{
  ...
  for (uint32_t i= len; i > 0; i--)
  {
    ...
    if (header->page.id().space() == SRV_TMP_SPACE_ID)
...
dberr_t fsp_xdes_reset(uint32_t space_id, uint32_t threshold, mtr_t *mtr)
{
  ...
  else memset(block->page.frame + descr_offset, 0,
             (last_descr_offset - descr_offset));
  return err;
}
...
func_exit:
...

  {
    ...
    {
      mtr.commit();
      sql_print_warning("InnoDB: Cannot shrink the temporary "
                        "tablespace from " UINT32PF" to "
                        UINT32PF " pages due to insufficient "
                        "innodb_buffer_pool_size", space->size,
                        last_used_extent);
    ...
    err= fsp_shrink_list(
           header, FSP_HEADER_OFFSET + FSP_FREE,
...
err_exit:
    ...
    if (err != DB_SUCCESS)
      goto err_exit;
    old_xdes_list.set_modify_blocks(&mtr);
  }
